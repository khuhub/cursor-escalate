...
- `npm install @cursor/sdk`; Node 22+, `"type": "module"`, tsconfig `module: NodeNext`, lib includes `ESNext.Disposable`. Run TS with `tsx`.
...
1. `npm run build` and `npm test` green across all workspaces; strict TS, no `any` in `core`.
...
7. **E2E smoke (real SDK, skipped without key):** run `cursor-looper "/goal make the failing test in examples/smoke-task pass"` against `examples/smoke-task` (a 3-file Node project with one intentionally failing vitest test and a `npm test` script) with ladder forced to a single cheap model; assert the loop reaches `passed`, the artifact has ≥1 iteration with a non-empty diff, and the rubric was generated by a real agent call.
